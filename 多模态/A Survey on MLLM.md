...
## LLM辅助的视觉推理
7.3.1 介绍
受工具增强型 LLMs 成功的启发，一些研究探索了在视觉推理任务中调用外部工具 或视觉基础模型 的可能性。这些研究将 LLMs 视为扮演不同角色的助手，构建了任务特定的 或通用的视觉推理系统。

与传统的视觉推理模型 [199], [200], [201] 相比，这些研究表现出以下几个显著优点：

强大的泛化能力：得益于从大规模预训练中获得的丰富开放世界知识，这些系统能够以显著的零样本或少样本性能轻松推广到未见过的对象或概念 [169], [170], [195], [197], [198], [202]。

新兴能力：借助 LLMs 强大的推理能力，这些系统能够执行复杂的任务。例如，给定一张图像，MMREACT [22] 可以解读其表面之下的深层含义，比如解释为什么某个表情包很有趣。

更好的交互性和可控性：传统模型通常只允许有限的控制机制，并且往往需要昂贵的定制数据集 [203], [204]。相比之下，基于 LLM 的系统可以通过用户友好的接口（如点击和自然语言查询）实现精细控制 [79]。

7.3.2 训练范式
免训练范式：

由于预训练的LLMs中存储了丰富的先验知识，一种直观且简单的方法是冻结预训练模型并直接提示LLMs以满足各种需求。根据设置，推理系统可以进一步分为少样本模型[22]、[169]、[170]、[181]和零样本模型[79]、[197]。少样本模型需要一些手工制作的上下文样本来引导LLMs生成程序或一系列执行步骤。这些程序或执行步骤作为相应基础模型或外部工具/模块的指令。零样本模型更进一步，直接利用LLMs的语言/语义知识或推理能力。

PointCLIP V2 [197]提示GPT-3生成具有3D相关语义的描述，以更好地与相应图像对齐。在CAT [79]中，LLMs被指示根据用户查询优化标题。

微调范式：

一些研究采用进一步的微调来提高系统在工具使用方面的规划能力[107]或增强其定位能力[142][205]。

GPT4Tools[107]引入了指令微调方法（见§3.2）。相应地，收集了一个新的与工具相关的指令数据集，并用于微调模型。

7.3.3 LLM在其中的功能
将现有工作中，LLM在LLM辅助视觉推理系统中所扮演的角色分为三类：

LLM作为Controller：LLM充当中央控制器，其职责包括：（1）将复杂任务分解为较简单的子任务/步骤；（2）将这些任务分配给适当的工具/模块。第一步通常通过利用LLM的CoT能力来完成。具体来说，LLM被明确提示输出任务规划[181]，或更直接地输出要调用的模块[107]、[169]、[170]。

VisProg[170]提示GPT-3输出一个视觉程序，其中每一行程序调用一个模块来执行子任务。此外，LLM还需要为模块输入输出参数名称。为了处理这些复杂需求，一些手工制作的上下文示例被用作参考[169]、[170]、[181]。这与推理链的优化（见§7.2）密切相关，或者更具体地说，与最少到最多提示[206]技术相关。通过这种方式，复杂问题被分解为按顺序解决的子问题。

LLM作为决策者： 在这种情况下，复杂任务以多轮方式解决，通常以迭代方式进行[195]。决策者通常履行以下职责：（1）总结当前上下文和历史信息，并决定当前步骤可用的信息是否足以回答问题或完成任务；（2）组织和总结答案，以用户友好的方式呈现。

LLM作为语义精炼器： 当LLM被用作语义精炼器时，研究人员主要利用其丰富的语言学和语义知识。具体来说，LLM通常被指示将信息整合为一致且流畅的自然语言句子[202]，或根据不同的特定需求生成文本[79]、[197]、[198]。
